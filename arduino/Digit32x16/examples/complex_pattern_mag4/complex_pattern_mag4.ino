...
/* complex pattern display for ikkeiplexing shield and Tichaino
    by ikkei 2012.3.18
*/
...
{PATT,0x0180,0x03C0,0x07E0,0x0DB0,0x0FF0,0x05A0,0x0810,0x0420},
...
void DisplayBitMap( byte pat_no )
{
  ...
  do{
    com = pgm_read_word_near (&BitMap[pat_no][frame][0]);
    if (com == PATT){
      for(byte line = 1; line < 9; line++) {
        uint16_t data = pgm_read_word_near (&BitMap[pat_no][frame][line]);
        for (byte led=0; led<16; ++led) {
          if (data & (1<<led)) {
            Digit32x16::Set(led*2,   line*2-2, 1);
            Digit32x16::Set(led*2,   line*2-1, 1);
            Digit32x16::Set(led*2+1, line*2-2, 1);
            Digit32x16::Set(led*2+1, line*2-1, 1);
          } else {
            Digit32x16::Set(led*2,   line*2-2, 0);
            Digit32x16::Set(led*2,   line*2-1, 0);
            Digit32x16::Set(led*2+1, line*2-2, 0);
            Digit32x16::Set(led*2+1, line*2-1, 0);
          }
        }
      }
      delay(blinkdelay);
    } else if (com == TIME){
      blinkdelay = pgm_read_word_near (&BitMap[pat_no][frame][1]);
    }
    frame++;
  } while (com != TERM);
}
...
void loop() {
  byte pattern = pgm_read_word_near (&seq[cnt]);
  if (pattern != PEND){
    DisplayBitMap( pattern );
    cnt++;
  } else {
    cnt = 0;
  }
